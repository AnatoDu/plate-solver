...
# Контракт симметрии входа. Каскад симметричный: ступени Якоби и спектральная
# работают с ½(A + Aᵀ), поэтому НЕсимметричный вход они решали бы молча и
# неверно. Пороги откалиброваны ИЗМЕРЕНИЯМИ двух хорошо разделённых популяций:
#   * округление сборки — кармановская касательная (гессиан симметричен ПО
#     ПОСТРОЕНИЮ) даёт ‖A−Aᵀ‖/‖A‖ до ~1.5e-8 на плохо обусловленных задачах
#     (конденсация плоских DOF через псевдообращение, масштабирование Якоби
#     переводит ошибку в относительную величину порядка ε·cond);
#   * содержательная асимметрия — член (B) полной КТН: 3.4e-2 на поставляемом
#     ci-случае, то есть на шесть порядков выше.
# Между ними — полоса неизвестного: там каскад считает, но ГРОМКО сообщает.
# Прежний единый порог 1e-10 стоял ВНУТРИ полосы округления и валил законные
# постановки Кармана-Ньютона (регрессия волны 0.8.0, найдена сверкой с 8544728).
#: ниже — округление сборки: молча
ASYM_ROUNDING = 1.0e-6
#: выше — вход содержательно несимметричен: отказ
ASYM_REFUSE = 1.0e-3
...
class SPDFactorization:
    ...
    def __init__(self, A, *, label: str = "A", rel_cutoff: float = REL_CUTOFF,
        ...
        # КОНТРАКТ СИММЕТРИИ (v0.8.0, откалиброван по регрессии). Ступени 2 и 3
        # работают с ½(A + Aᵀ): для НЕсимметричного входа они решают
        # симметризованную задачу, и при полном ранге симметричной части это
        # происходило МОЛЧА (касательная КТН-Ньютона, ошибка шага до 5 %).
        # Реакция ГРАДУИРОВАНА по измеренным популяциям (см. ASYM_ROUNDING /
        # ASYM_REFUSE): округление сборки пропускается молча, промежуточная
        # полоса — громкое предупреждение (счёт продолжается), содержательная
        # асимметрия — отказ с указанием правильного инструмента.
        asym = float(np.max(np.abs(A - A.T)))
        scale = float(np.max(np.abs(A)))
        ratio = asym / scale if scale > 0.0 else 0.0
        if ratio > ASYM_REFUSE:
            raise np.linalg.LinAlgError(
                f"{self.label}: матрица НЕсимметрична (‖A−Aᵀ‖/‖A‖ = "
                f"{ratio:.2e} > {ASYM_REFUSE:.0e}) — симметричный каскад к "
        ...
        if ratio > ASYM_ROUNDING:
            self._emit(warn, (
                f"вход асимметричен на {ratio:.2e} — это выше полосы округления "
                f"({ASYM_ROUNDING:.0e}), но ниже порога отказа "
                f"({ASYM_REFUSE:.0e}); ступени 2 и 3 каскада решают задачу с "
                "½(A+Aᵀ). Если асимметрия содержательна, а не от округления, "
                "используйте несимметричный решатель (ktn_full._lin_solve)"))
