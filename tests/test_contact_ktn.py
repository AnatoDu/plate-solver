...
def test_pair_faces_are_mirror_antisymmetric():
    r"""ВОРОТА КОНВЕНЦИИ ГРАНЕЙ: при ``f₂ = −f₁`` лицевые пары антисимметричны.

    Две ОДИНАКОВЫЕ пластины с одинаковыми краевыми условиями, нагруженные
    навстречу друг другу (``f₂ = −f₁``), образуют зеркально-симметричную
    конфигурацию: изгиб выходит антисимметричным БИТ-В-БИТ (``w₂ = −w₁``).
    Тогда и лицевые обязаны удовлетворять ``u_{c2} = −u_{c1}`` — контактирующая
    грань второй пластины есть зеркальный образ контактирующей грани первой.

    Ворота ловят знак члена собственной нагрузки: при ``+κ_q₂ f₂`` (запись
    v0.8.0 до аудита) тождество ломалось ровно на ``2κ_q f₁`` — до 45 % от
    ``max|u_c|`` в этой постановке. Член реакции при этом входит со знаком
    «плюс» законно (податливости граней складываются) и антисимметрии не
    нарушает: он одинаков у обеих пластин, а ``r`` — не знаковая нагрузка,
    а неотрицательное давление.
    """
    cfg = _cfg(max_iter=400)
    s1, s2 = _two_solvers(cfg)                          # одинаковые пластины
    f1 = np.full(s1.quad.x.size, float(cfg.q0))
    mor = NonlinearTwoPlateMOR(s1, s2, cfg, gap=0.0, f1=f1, f2=-f1)
    res = mor.solve()
    u1, u2 = mor.faces(res.cw1, res.cw2, res.r_nodes)
    scale = float(np.max(np.abs(u1)))
    # изгиб зеркален по построению задачи (одинаковые операторы, встречные нагрузки)
    assert np.max(np.abs(res.w1_nodes + res.w2_nodes)) < 1e-12 * float(
        np.max(np.abs(res.w1_nodes)))
    # и лицевые обязаны быть антисимметричны — это и есть проверяемая конвенция
    assert np.max(np.abs(u1 + u2)) < 1e-10 * scale, "нарушена зеркальная антисимметрия лицевых"
    # ворота невакуумны: члены лицевого условия ненулевые
    assert mor._kappa_q2 > 0.0 and mor._kappa_r2 > 0.0
    assert scale > 0.0
