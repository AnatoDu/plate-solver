...
def _check_union_overlap(tree: dict, dom, *, probe: int = 257) -> None:
    ...
    :data:`NARROW_OVERLAP_FRACTION` — порог поставлен по ХУДШЕМУ краевому
    условию (защемление), поэтому на мягком шарнире предупреждение
    консервативно: там при ширине 0.20 отклонение всего −0.7 %. ОСТАТОЧНАЯ
    погрешность в несколько процентов сохраняется и при широком перекрытии: составная ``ω`` — иная
    ...
    def walk(node: dict) -> None:
        ...
        if np.isfinite(min_width) and min_width < NARROW_OVERLAP_FRACTION * size:
            warnings.warn(
                ...
                "объединении двух полос в единичный квадрат, p = 8; ЗАЩЕМЛЕНИЕ: "
                "−87 % при ширине 0.03, −84 % при 0.05, −18 % при 0.10, −8 % при "
                "0.20; мягкий шарнир мягче: −63 %, −30 %, −3 %, −0.7 % — порог "
                "поставлен по худшему из краевых условий), обусловленность "
                "падает. Увеличьте перекрытие; там, где область выражается "
                "ОДНИМ примитивом, задавайте её примитивом.",
