...
@pytest.mark.parametrize("bad", [
    ...
    "9**(9**(9**9))",                # ТА ЖЕ башня в скобках (аудит 0.8.0)
    "9**(1+8)**9",                   # башня со скобками в показателе
    "2**(3**(4**(5**6)))",           # глубокая скобочная башня
    "(9**60)**60",                   # без вложенности, но числовой взрыв
    "((9**60)**60)**60",
...
def test_expr_resource_fence(bad):
    ...
    Скобочные башни (``9**(9**(9**9))``) прежнюю ограду ПРОХОДИЛИ: она
    смотрела на пару смежных токенов, а чтение case-файла подвисало на разборе
    более чем на полторы минуты (аудит 0.8.0). Ограда степеней перенесена на
    дерево ``ast``: показатель, вложенность и порядок числовой степени.
    """
    import time
...
@pytest.mark.parametrize("good", [
    "1.0 + 0.3*x**2 + y**3",         # многочлен: степени законны
    "(x**2 + y**2)**0.5",            # дробный показатель
    "x**64",                         # предельный допустимый целый показатель
    "(x**8)**8",                     # вложенность в ОСНОВАНИИ (не башня)
    "(2**10)*x",                     # числовая степень умеренного порядка
    "exp(-((x - 0.2)**2 + y**2)/0.05)",
    "x**(y**2)",                     # СИМВОЛЬНАЯ башня: гигантских целых не даёт
    "2.0**(x**2)",                   # и в основании число — тоже законно
])
def test_expr_fence_admits_legitimate_powers(good):
    """Ограда степеней НЕ трогает законные записи (иначе она бесполезна)."""
    from plate_solver.exprfield import parse_field

    assert parse_field("load.expr", good) is not None
