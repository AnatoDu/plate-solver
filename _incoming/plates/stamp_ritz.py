...
Та же 1D-контактная задача, что и в ``plates.stamp`` (функция Грина), но изгиб на
...
``plates.stamp`` (нодальная реакция ``r := max(0, r + β(w − Δ))`` под штампом).
"""
...
# --------------------------------------------------------------------------- #
#  МОР поверх оператора отклика (тот же алгоритм, что в plates.stamp)
# --------------------------------------------------------------------------- #
