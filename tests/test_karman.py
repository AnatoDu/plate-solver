...
def test_picard_step_norm_is_not_residual():
    ...
    Одно и то же число ``karman_tol`` под двумя критериями задаёт разную
    точность, и сравнивать с ним норму шага (как делал Пикар до v0.8.0, тогда
    как Ньютон — невязку) некорректно: шаг невязку НЕ мажорирует. Шаг сохранён
    4-м элементом ``history`` как диагностика.

    Ворота смотрят на ВСЮ историю, а не на последнюю запись. Последняя точка
    непортируема: число итераций определяется тем, с какой стороны от ``tol``
    окажется невязка на предпоследнем шаге (здесь до порога 1.3×), и другая
    арифметика — иной BLAS или просто перестановка узлов квадратуры —
    переводит останов на следующую итерацию, где норма шага сразу меньше в
    ~28 раз. Проверено: на OpenBLAS прежняя запись падала детерминированно, на
    Accelerate — на 3 перестановках узлов из 6. Максимум отношения по истории
    устойчив: 25.0 (эта машина), 6.4…142 на перестановках, 8.3 на OpenBLAS —
    худший запас 2.1× при пороге 3.0.
    """
    tol = 1e-6
    _, r, res = _picard_circle(200.0, Q=48, p=8, n_load_steps=4, tol=tol)
    assert r.converged and res <= tol
    ratios = [dw / rn for _frac, _it, rn, dw in r.history if rn > 0.0]
    assert ratios, "история Пикара пуста — проверять нечего"
    assert max(ratios) > 3.0, ratios     # шаг где-то ПО ХОДУ на порядок больше невязки
    # и обратное: невязка в точке останова мала, а шаг — нет обязан быть малым
    assert r.history[-1][2] <= tol
