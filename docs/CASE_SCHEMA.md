...
| circle + контакт | soft_hinge | uniform (основание, скалярный Δ) | сертифицированный осесимметр. контакт Кирхгофа (плоская зона + кольцевая реакция); гейт — прогиб ВНЕ зоны, `w_max = Δ` тривиален |

Контакт: `analytic` доступен ТОЛЬКО для круг + `soft_hinge` + `classic` +
основание + скалярный `gap` (0 < Δ < w_free(0)); прочий контакт — ворота
инвариантов (`reference = none`). Вне таблицы (L, compose, patch на кривых
зонах) — `mms` | `fem` | `none`: свободная граница контакта на произвольной
