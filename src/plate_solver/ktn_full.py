...
class KTNPlate(KarmanPlate):
    ...
    def fallback(self) -> str | None:
        """Деградация внеплоскостного решателя: ``None`` | ``"lstsq"`` | ``"sym-tangent"``.

        ``None`` — все шаги Пикара/Ньютона прошли штатным LU; ``"lstsq"`` —
        хотя бы на одном шаге LU дал нечисловое решение и включился МНК
        (пространство усечено, см. :func:`_lin_solve`); ``"sym-tangent"``
        (v0.8.0) — ньютоновский шаг взят по СИММЕТРИЗОВАННОЙ касательной как
        квази-Ньютон, потому что линейный шаг по исходной матрице не был решён
        (проверка невязки, :data:`_TANGENT_RESID_TOL`).
        """
