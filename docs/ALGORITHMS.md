...
`u_c = w + (h_c²−h_*²)Δw`, коэффициент кривизны масштабируется теорией.
...
шаг `β_eff = β/gain` (нормировка `gain_mode`: `secant` — `gain = w_free/q0`,
секущая податливость в рабочей точке, по умолчанию; `linear` —
`gain = w_max_classic/q0`, линейная податливость при неподвижной кромке: верхняя
грань ‖G‖ ⇒ равномерное сжатие T4, нужна при `w_lin/w_free ≫ 1`, где секущая
занижает ‖G‖ у контактного состояния), недорелаксация `θ`, критерий останова.
...
повторять до сходимости (совмещённая схема):
    ...
    u_c ← w(c) + (h_c²−h_*²)·Δw(c)                 # лицевой прогиб
...
кармановский контакт (`u_c = w`). Реализация: `contact_nl.py`; диагностика зоны
(размер, пик, сила, число связных пятен) — `diagnostics.py`.
...
дополнением выреза, `r_and`/`r_diff` в `rfunctions.py`); структура `w = ω^s·Φ`
удовлетворяет ГУ на ВСЕХ компонентах границы автоматически. Решатели теорий и
контакт работают без изменений — многосвязность целиком в геометрии.
