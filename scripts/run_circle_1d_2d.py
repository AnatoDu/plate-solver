...
Осесимметричная пластина решена 1D-методом Ритца по радиусу (``plate_solver.radial``) и
сверена с радиальным сечением 2D-решения (RFM+Ритц) и с точной формулой —
верификация 1D ↔ 2D ↔ аналитика (осевая симметрия). a=1, q=4, ν=0.3.
...
from plate_solver.radial import RadialClamped, solve_radial_soft_hinge
