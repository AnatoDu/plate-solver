...
def contact_invariant_rows(result: Result) -> list[RefRow]:
    ...
    * ПРОНИКАНИЕ ``max(u − Δ)₊/Δ ≤ 5 %`` — условие непроникания выполняется
      ...
      относительному перелёту свободного решения и заведомо велико. При
      ``Δ = 0`` (касание — так заданы все поставляемые пары) доля зазора не
      определена, и величина нормируется масштабом прогиба; строка отчёта
      называет фактическую нормировку;
    * НЕТРИВИАЛЬНОСТЬ: если свободное решение перекрывает зазор В ЗОНЕ
      основания (``Result.free_overlap > 0``), контакт ОБЯЗАН существовать
      (``n_contact > 0``); если не перекрывает — реакции быть НЕ ДОЛЖНО.
      Величина меряется по ТОЙ ЖЕ поверхности, по которой стоит условие
      непроникания, и только под основанием (для пары — по СБЛИЖЕНИЮ лицевых):
      глобальный ``max|w_free|`` давал ложный отказ при штампе у кромки и у
      пары одинаковых пластин, а при зазоре больше прогиба первой пластины —
      наоборот, снимал единственные нетривиальные ворота (аудит 0.8.0);
    * СИЛОВОЙ режим: замыкание ``|∫r − P|/P ≤ 2 %``.
    ...
    if np.isfinite(over):
        pen = max(over, 0.0)
        # НОРМИРОВКА зависит от постановки: при Δ > 0 это доля зазора, а при
        # Δ = 0 (касание, пары в поставке) доля зазора не определена и берётся
        # масштаб прогиба. Имя строки называет фактическую нормировку —
        # прежде обе величины подписывались «доля зазора» (аудит 0.8.0).
        by_gap = result.delta is not None and float(result.delta) > 0.0
        name = ("инвариант: проникание (доля зазора)" if by_gap
                else "инвариант: проникание (доля масштаба прогиба, Δ = 0)")
        rows.append(RefRow(name=name, reference=0.0, value=pen, rel=pen,
                           gated=True, passed=pen <= PENETRATION_TOL))
    n_contact = int((c.r_nodes > 0).sum())
    if result.free_overlap is not None and np.isfinite(result.free_overlap):
        # Ожидаемость касания меряется ПО ЗОНЕ основания и по ТОЙ ЖЕ
        # поверхности, по которой стоит условие непроникания (для пары — по
        # сближению лицевых). Глобальный max|w_free| для этого не годился:
        # при штампе у кромки он требовал контакта там, где верный ответ
        # r ≡ 0, а у пары одинаковых пластин сближение тождественно нулевое
        # при любом прогибе (аудит волны 0.8.0).
        # у самой границы касания (перекрытие ≈ 0) решение неустойчиво к шагу
        # квадратуры — там не гейтуется НИ ОДНА из сторон
        band = 1.0e-6 * abs(float(result.delta)) if result.delta else 0.0
        ov = float(result.free_overlap)
        if ov > band:
            rows.append(RefRow(name="инвариант: контакт существует (перекрытие зазора)",
                               reference=1.0, value=float(n_contact > 0),
                               rel=0.0 if n_contact > 0 else 1.0,
                               gated=True, passed=n_contact > 0))
        elif ov < -band:
            # обратная сторона того же инварианта: свободное решение зазор не
            # перекрывает ⇒ реакции быть НЕ ДОЛЖНО (ловит паразитный контакт)
            rows.append(RefRow(name="инвариант: контакта нет (зазор не перекрыт)",
                               reference=0.0, value=float(n_contact),
                               rel=float(n_contact > 0),
                               gated=True, passed=n_contact == 0))
