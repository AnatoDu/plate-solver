...
верхняя лицевая НИЖНЕЙ (валидатор отклоняет КТН-теории (`ktn_linear`/`ktn_full`)
для пары с этим пояснением). Канон напряжений (§19, NOTES): у верхней пластины реакция
