...
def test_gate_r3_reduction_1d_ktn_off_is_karman():
    """Gate R3 (редукция 1D): радиальная КТН с ВЫКЛЮЧЕННЫМИ членами — это Карман.

    Прежняя редакция сравнивала «выключено» с «включено» и проверяла лишь
    АКТИВНОСТЬ членов, но не саму редукцию (имя теста ей не соответствовало).
    Редукция удостоверяется двумя признаками, независимыми от кода решателя:

    * ЭТАЛОН. При выключенных членах решение обязано лечь на ряды Кармана для
      защемлённого круга с несмещаемой кромкой (в публикации: Way (1934),
      ν = 0.3, ``benchmarks.way_clamped_circle``) — вся таблица в пределах
      5·10⁻³ (факт ≤ 4.7·10⁻³; у 2D-решателя на быстрой сетке — 2·10⁻²);
      в линейном пределе (P̄ = 0.02) — на замкнутую форму Кирхгофа.
    * ПОДОБИЕ. У Кармана безразмерный прогиб ``w₀/h`` зависит ТОЛЬКО от
      ``P̄ = q/h⁴`` (при E = a = 1) — при h = 0.2 и h = 0.05 он совпадает
      бит-в-бит. КТН-поправка порядка ``O(h²/L²)``, поэтому любая её утечка в
      «выключенную» ветвь ломает подобие: с ВКЛЮЧЁННЫМИ членами расхождение по
      h составляет 0.5…2.8 % (контроль ниже) — тест не вакуумен.
    """
    for P_bar, w_way in bm.way_clamped_circle():
        wh = {}
        for h in (0.2, 0.05):
            _, r = _radial(h, P_bar, include_ktn=False)
            assert r.converged, (P_bar, h)
            wh[h] = r.w_max / h                       # безразмерный прогиб w₀/h
        assert wh[0.2] == wh[0.05], (P_bar, wh)       # подобие Кармана (бит-в-бит)
        assert abs(wh[0.05] - w_way) / w_way < 5e-3, (P_bar, wh[0.05], w_way)

    # линейный предел: Карман → Кирхгоф (замкнутая форма), факт ~6e-6
    _, r_lin = _radial(0.1, 0.02, include_ktn=False)
    assert abs(r_lin.w_max / 0.1 - bm.kirchhoff_clamped_circle(0.02, 0.3)) \
        / bm.kirchhoff_clamped_circle(0.02, 0.3) < 1e-4

    # контроль: с ВКЛЮЧЁННЫМИ членами оба признака нарушаются (члены активны)
    on = {}
    for h in (0.2, 0.05):
        _, r = _radial(h, 6.321, include_ktn=True)
        assert r.converged
        on[h] = r.w_max / h
    assert abs(on[0.2] - on[0.05]) / on[0.05] > 1e-3          # подобие сломано
    _, r_off = _radial(0.2, 6.321, include_ktn=False)
    assert abs(on[0.2] - r_off.w_max / 0.2) / (r_off.w_max / 0.2) > 1e-3
