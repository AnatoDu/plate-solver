...
# --------------------------------------------------------------------------- #
def test_linear_ktn_face_modes_agree_and_pointwise_is_regularized():
    """Линейная КТН: обе формы лицевого условия дают одно решение по прогибу;
    поузельная форма БОЛЬШЕ НЕ РВЁТ реакцию (v0.8.0).

    До v0.8.0 член ``κ_r·r`` умножался на цилиндрическую жёсткость: в единицах
    этой фикстуры (E = 1, h = 0.2 ⇒ D = 7.3e-4) он был подавлен в ~1400 раз, и
    поузельное второе дифференцирование давало «провал» реакции (звон ``Δw``)
    между центром и кромочным кольцом — его заполняла слабая форма (целевой
    эффект прототипа). После исправления размерности κ_r сам регуляризует
    поузельную форму: провала нет (``interior_min > 0``), и целевой эффект
    слабой формы при этих параметрах не наблюдается.

    Остаточное различие форм — РАДИУС зоны (≈ 12 %): оно НЕ убывает с Q
    (измерено при Q = 64, 96, 128, 160: 0.046, 0.056, 0.043, 0.044), то есть
    это различие двух дискретизаций лицевого условия, а не квадратурный
    артефакт — цена реформулировки (прототип, вне публичного фасада).
    """
    cfg = _cfg(Q=96)
    s = _lin_solver(cfg)
    ...
    # прогиб и плато — совпадают (внутренняя согласованность, §6.1)
    assert abs(pt.w_max - wk.w_max) / pt.w_max < 2e-2
    # радиус зоны различается, но ограниченно и без роста с Q
    assert abs(pt.rho_c - wk.rho_c) < 0.10
    assert pt.rho_c > 0.3 and wk.rho_c > 0.3
    ...
    # поузельная форма регуляризована исправленным κ_r: провала к нулю нет
    assert interior_min(pt) > 0.1 * pt.r_max
    assert interior_min(wk) > 0.0
